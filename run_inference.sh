...
i=$((i+1))
    ...
    do
        python scripts/classification_inference.py -f datasets/$dataset_category/$dataset_names/split/$split_type/$filename -sc smiles -t label -tp classification_results/$dataset_category/$dataset_names/$split_type/$model_name/$i -ip classification_results/$dataset_category/$dataset_names/$split_type/$model_name/$i --soft-classification 
    done
done